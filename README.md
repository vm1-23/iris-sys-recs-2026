...
Configured NGINX reverse proxy to controll access and balance load.
Instead of exposing the app directly, NGINX acts as the only public door into the cluster.
...
**ISSUE** I faced here was Docker accidentally mounting the raw .htpasswd file as a directory, causing NGINX to completely crash with 500 errors. I fixed this by having the password file inside an `auth/` host folder and mounting the entire directory instead. I also explicitly configured NGINX to wipe the HTTP Authorization header manually before proxying traffic backward, because Grafana was accidentally rejecting admin logins with a 403 Forbidden error.
...
**ISSUE** here was a brutal Arch Linux system conflict. When Docker tried to natively mount the NFS volume, it threw "operation not supported" because my system lacked `nfs-utils`. I made a standard package overwrite to install the utilities, allowing Docker to mount the volume. i also paused the boot cycle, ensuring the NFS container starts up 5 seconds before the web layer to eliminate any early-mount race conditions.
...
**During implementation,** the Rails app middleware crashed when I added the Prometheus tracking gem, throwing 500 Route mapping errors. I fixed this by writing a custom initializer to unshift the tracking interceptor to the top of the Rack load order!

## 4. Automated Backup System
I made an entirely standalone Cron container packaged on Ubuntu 22.04.

The script wakes up securely every midnight and immediately verifies the MySQL host is online. Upon verification, it seamlessly executes `mysqldump` for the database layer, and uses `tar` to cleanly compress the securely mounted read-only NFS shared volume. Before closing, the script runs a chronological pruning command (keep only the last 5 files) to guarantee that the disk never strictly overflows with stale backups. The archives are piped out to the local host's `./backups` folder.

**ISSUE** here was that the original Alpine OS container fatally rejected MySQL 8.0's `caching_sha2_password` protocol,causing the database to reject the Ping authentication attempt completely. Once I actively swapped the Dockerfile to use Ubuntu natively, the Oracle `mysql-client` processed the authentication hash without a single issue.
